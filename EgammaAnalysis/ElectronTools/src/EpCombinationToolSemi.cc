...
/*****************************************************************/
std::pair<float, float> EpCombinationToolSemi::combine(reco::GsfElectron& electron, float calibratedEnergy, float calibratedResolution) const
/*****************************************************************/
{

  float combinedEnergy = calibratedEnergy;
  float combinedEnergyError = calibratedResolution;
  ...
  const float eOverP           = calibratedEnergy/trkMomentum;
  ...
  if (calibratedEnergy < highEnergyThr &&
      eOverP > eOverPThr && 
      std::abs(calibratedEnergy - trkMomentum) < epDiffSigThr*std::sqrt(pModeError*pModeError+calibratedResolution*calibratedResolution) && 
      trkMomentumError < epSigThr*trkMomentum) { 

    bool iseb = electron.isEB();
    float raw_pt = calibratedEnergy/cosh(trkEta);
    ...
    eval[0] = calibratedEnergy;
    eval[1] = calibratedResolution/calibratedEnergy;
    ...
    float rawcomb = ( calibratedEnergy*trkMomentumError*trkMomentumError + trkMomentum*calibratedResolution*calibratedResolution ) / ( trkMomentumError*trkMomentumError + calibratedResolution*calibratedResolution );
